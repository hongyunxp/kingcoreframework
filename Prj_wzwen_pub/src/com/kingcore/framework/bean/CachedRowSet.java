/**
 * Copyright (C) 2007 Zeven.Wu. All rights reserved.
 * Zeven PROPRIETARY/CONFIDENTIAL. Use is subject to license terms.
 */
...
public class CachedRowSet implements RowSet,DataSet {
	...
	/**
	 * 采用　List(Vector,ArrayList,Stack etc) 构建 CachedRowSet对象
	 */
	...
	/**
	 * 采用 ResultSet/RowSet 构建 CachedRowSet对象
	 * 
	...
	public CachedRowSet(ResultSet rs) {
		...
		try{   
			...
			for(int   j=1;j<=rsmd.getColumnCount();j++){   
				currentRow.put( j+"" , rsmd.getColumnLabel(j).toLowerCase() );   
			}
			...
			if   (rsmd.getColumnCount()>=0){     
				while(rs.next()){
					...
					for(int   j=1;j<=rsmd.getColumnCount();j++){ 
						temph.put( rsmd.getColumnLabel(j).toLowerCase() , rs.getObject(j)==null?"":rs.getObject(j) ) ;   
					}   
					list.add( temph );   
				}   
			}  
...
//		rowNumber=-1;	
	...
	public int getInt(int columnIndex) throws SQLException {
		String columnName = (String) currentRow.get( columnIndex+"" );
	...
	public int getInt(String columnName) throws SQLException {
		//System.out.println(this.currentRow.get(columnName.toLowerCase()) + "-----");
		//String temp = (String)this.currentRow.get(columnName.toLowerCase());
